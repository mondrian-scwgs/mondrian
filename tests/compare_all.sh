...
ls $CODEBUILD_SRC_DIR/tests/breakpoint_calling/outputs/results/
...
docker run -w $PWD -v $PWD:$PWD -v $CODEBUILD_SRC_DIR:$CODEBUILD_SRC_DIR quay.io/mondrianscwgs/alignment:${TAG}beta mondrian_build_utils compare_breakpoint_calling \
    --destruct $CODEBUILD_SRC_DIR/tests/breakpoint_calling/outputs/results/extract_somatic_destruct_somatic_breakpoints.csv --destruct_ref result_reference/destruct.csv \
